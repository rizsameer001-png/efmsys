...
//       if (response.data.success) {
//         setLogs(response.data.data.logs || []);
//         setPagination(response.data.data.pagination || { page: 1, limit: 50, total: 0, pages: 0 });
//       }
//     } catch (error) {
//       console.error('Fetch audit logs error:', error);
//       showToast(error.userMessage || 'Failed to load audit logs', 'error');
//       setLogs([]);
//     } finally {
//       setLoading(false);
//     }
...
//           byAction: response.data.data.byAction || [],
//           byUser: response.data.data.byUser || []
//         });
//       }
//     } catch (error) {
//       console.error('Fetch stats error:', error);
...
//       showToast(error.userMessage || 'Failed to export audit logs', 'error');
//     } finally {
//       setExporting(false);
//     }
//   };
...
//         </Card>
//         <Card className="p-4 text-center">
//           <p className="text-2xl font-bold text-purple-600">{stats.thisWeek || 0}</p>
//           <p className="text-sm text-gray-500">This Week</p>
//         </Card>
//         <Card className="p-4 text-center">
...
//                     </td>
//                     <td className="px-6 py-4 whitespace-nowrap">
//                       <div className="text-sm font-medium text-gray-900">{log.userName || log.username || 'Unknown'}</div>
//                       <div className="text-xs text-gray-500">{log.userRole || log.role || 'User'}</div>
//                     </td>
//                     <td className="px-6 py-4 whitespace-nowrap">
...
//             stats.byUser.slice(0, 5).map((user, idx) => (
//               <div key={user._id || user.id || idx} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
//                 <div>
//                   <p className="font-medium text-gray-900">{user.name || user.username || 'Unknown'}</p>
//                   <p className="text-sm text-gray-500 capitalize">{user.role || 'User'}</p>
//                 </div>
//                 <div className="text-right">
//                   <p className="text-lg font-bold text-blue-600">{user.count || 0}</p>
//                   <p className="text-xs text-gray-500">actions</p>
//                 </div>
//               </div>
//             ))
...
//       {/* Top Actions Section */}
//       <Card className="p-6">
//         <h3 className="font-semibold text-gray-900 mb-4">Most Common Actions</h3>
//         <div className="space-y-3">
//           {stats.byAction && stats.byAction.length > 0 ? (
//             stats.byAction.slice(0, 5).map((action, idx) => (
//               <div key={action._id || action.action || idx} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
//                 <div>
//                   <p className="font-medium text-gray-900">{action.action || action._id || 'Unknown'}</p>
//                 </div>
//                 <div className="text-right">
//                   <p className="text-lg font-bold text-purple-600">{action.count || 0}</p>
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {logs && logs.length > 0 ? (
                logs.map((log, index) => (
                  <tr key={log._id || log.id || index} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {formatDate(log.timestamp || log.createdAt)}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
      ...
      <Card className="p-6">
        <h3 className="font-semibold text-gray-900 mb-4">Most Active Users</h3>
        <div className="space-y-3">
          {stats.byUser && Array.isArray(stats.byUser) && stats.byUser.length > 0 ? (
            stats.byUser.slice(0, 5).map((userItem, idx) => (
              <div key={userItem._id || userItem.id || idx} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                <div>
                  <p className="font-medium text-gray-900">{userItem.name || userItem.username || 'Unknown'}</p>
                  <p className="text-sm text-gray-500 capitalize">{userItem.role || 'User'}</p>
                </div>
                <div className="text-right">
                  <p className="text-lg font-bold text-blue-600">{userItem.count || 0}</p>
                  <p className="text-xs text-gray-500">actions</p>
                </div>
              </div>
            ))
      ...
      <Card className="p-6">
        <h3 className="font-semibold text-gray-900 mb-4">Most Common Actions</h3>
        <div className="space-y-3">
          {stats.byAction && Array.isArray(stats.byAction) && stats.byAction.length > 0 ? (
            stats.byAction.slice(0, 5).map((actionItem, idx) => (
              <div key={actionItem._id || actionItem.action || idx} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                <div>
                  <p className="font-medium text-gray-900">{actionItem.action || actionItem._id || 'Unknown'}</p>
                </div>
                <div className="text-right">
                  <p className="text-lg font-bold text-purple-600">{actionItem.count || 0}</p>
                  <p className="text-xs text-gray-500">occurrences</p>
                </div>
              </div>
            ))
