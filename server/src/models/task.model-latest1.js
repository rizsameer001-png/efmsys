...
taskSchema.pre('save', function(next) {
  ...
  if (this.slaDeadline && this.slaDeadline < new Date() && !this.slaBreached) {
...
// ==================== PRE-FIND MIDDLEWARE ====================
// Fix: Properly exclude soft-deleted tasks from all find queries
const excludeDeleted = function() {
  this.where({ isDeleted: { $ne: true } });
};

taskSchema.pre('find', excludeDeleted);
taskSchema.pre('findOne', excludeDeleted);
taskSchema.pre('findOneAndUpdate', excludeDeleted);
taskSchema.pre('countDocuments', excludeDeleted);

// ==================== STATIC METHODS ====================

/**
 * Find overdue tasks (SLA breached)
 */
...
taskSchema.statics.getStatusCounts = async function() {
  try {
    ...
    const result = {
      ...
      waiting_parts: 0, completed: 0, verified: 0, closed: 0, cancelled: 0
    };
